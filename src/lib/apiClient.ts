...
export const apiClient = {
  ...
  asistencia: {
    ...
    guardar(body: { asignatura: string; grado: string; salon: string; fecha: string; marcas: { estudiante_id: string; estado: AsistenciaEstado }[] }): Promise<{ ok: true; marcas: { estudiante_id: string; estado: AsistenciaEstado; auto_excusa: boolean }[] }> {
      return request('/api/asistencia/guardar', { method: 'POST', body: JSON.stringify(body) });
    },
  ...
  estadisticas: {
    ...
    institucional(periodo: number | 'anual', esquema?: string, nivel?: string): Promise<ApiInstitucional> {
      return request<ApiInstitucional>(`/api/estadisticas/institucional${qs({ periodo, esquema, nivel })}`);
    },
    ...
    riesgo(periodo: number | 'anual', umbral?: number, grado?: string, salon?: string, asignatura?: string, esquema?: string, nivel?: string): Promise<ApiRiesgo> {
      return request<ApiRiesgo>(`/api/estadisticas/riesgo${qs({ periodo, umbral, grado, salon, asignatura, esquema, nivel })}`);
    },
  ...
  orientacion: {
    ...
    remisionAtendida(remision_id: number): Promise<{ ok: true; atendida_por_nombre: string; atendida_at: string }> {
      return request('/api/orientacion/remision-atendida', { method: 'POST', body: JSON.stringify({ remision_id }) });
    },
    remisionesSinRevisar(): Promise<{ ids: number[] }> {
      return request('/api/orientacion/remisiones-sin-revisar');
    },
