...
const JustificacionUniformeStaff = () => {
  ...
  const justFiltradas = visibles.filter(j => {
    ...
    if (!coincideBusqueda(busqueda, j.estudiante_nombre, j.estudiante_apellidos, j.estudiante_id)) return false;
    return true;
  });
  ...
  const handleDescargar = async () => {
    ...
    try {
      ...
      for (const j of justFiltradas) {
        ...
        if (j.acudiente_telefono) acudienteFila.push({ label: "Teléfono:", value: formatTelefono(j.acudiente_telefono) });
        secciones.push({
