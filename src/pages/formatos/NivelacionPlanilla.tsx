...
const NivelacionPlanilla = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setFilas(ests.map((e) => ({ id: String(e.id), nombre: `${e.apellidos} ${e.nombres}`.trim(), nota: "", obs: "" })));
      setCargando(false);
    })();
  }, [grado, salon]);
  ...
  const armarDatos = () => ({ tipo: "nivelacion", docente, asignatura, periodo, fecha, grado, salon, filas, firma });
  ...
  const descargarPDF = async () => {
    ...
    const drawHead = () => {
      ...
      cols.forEach(([t, w]) => { d.text(t, x + 1.5, y + 5); x += w; if (x < W - M) d.line(x, y, x, y + rowH); });
    ...
    filas.forEach((f, i) => {
      ...
      cols.forEach(([, w], ci) => { const txt = d.splitTextToSize(String(cells[ci]), w - 3)[0] || ""; d.text(txt, x + 1.5, y + 5); x += w; if (x < W - M) d.line(x, y, x, y + rowH); });
      y += rowH;
    });
    y += 12;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <p className="text-muted-foreground mt-1 text-sm">Elige grado y salón (se cargan los estudiantes), escribe las notas, firma y guarda.</p>
            ...
            <div className="overflow-x-auto border border-border rounded-lg">
              <table className="w-full text-sm">
                <thead className="bg-muted/50"><tr>
                  ...
                  <th className="p-2 text-left w-28">Nota definitiva</th><th className="p-2 text-left">Observaciones</th>
                </tr></thead>
                <tbody>
                  {filas.map((f, i) => (
