        ...
        <div className="mt-6 space-y-4 bg-card rounded-lg shadow-soft p-5">
          <div className="grid md:grid-cols-3 gap-3">
            <div><label className="text-sm font-medium">Asignatura *</label>
              <select value={asignatura} onChange={(e) => { setAsignatura(e.target.value); setGrado(""); setSalon(""); }} className={inputCls + " cursor-pointer"}>
            ...
            <div><label className="text-sm font-medium">Grado *</label>
              <select value={grado} onChange={(e) => { setGrado(e.target.value); setSalon(""); }} disabled={!asignatura} className={inputCls + " cursor-pointer disabled:opacity-60"}>
            ...
            <div><label className="text-sm font-medium">Salón *</label>
              <select value={salon} onChange={(e) => setSalon(e.target.value)} disabled={!grado} className={inputCls + " cursor-pointer disabled:opacity-60"}>
          ...
          {grado && salon && (
            <div className="flex flex-wrap items-end gap-2 border-t border-border pt-4">
              <div className="flex-1 min-w-[200px]">
                ...
                <select value={porAgregar} onChange={(e) => setPorAgregar(e.target.value)} disabled={cargandoPool} className={inputCls + " cursor-pointer disabled:opacity-60"}>
              ...
              <button type="button" onClick={agregar} disabled={!porAgregar} className="inline-flex items-center gap-1 px-4 py-2 rounded-md bg-primary text-primary-foreground font-medium hover:bg-primary/90 disabled:opacity-50"><Plus className="w-4 h-4" /> Agregar</button>
            </div>
          )}
                ...
                <tbody>
                  {filas.map((f, i) => (
                    <tr key={f.id} className="border-t border-border">
                      ...
                      <td className="p-2"><input value={f.nota} onChange={(e) => setFila(i, "nota", e.target.value)} className="px-2 py-1 border border-input rounded w-20 bg-background" /></td>
                      ...
                      <td className="p-2">
                        {f.firma
                          ...
                          : <button type="button" onClick={() => abrirFirma(i)} className="text-xs px-3 py-1 rounded border border-primary text-primary hover:bg-primary/5">Firmar</button>}
                      </td>
