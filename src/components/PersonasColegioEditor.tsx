import { useEffect, useRef, useState, useLayoutEffect, useMemo, type CSSProperties } from "react";
...
const PersonasColegioEditor = ({ colegioId, rol: rolProp, setRol: setRolProp, onChanged }: Props) => {
  ...
  // Estilo de grilla con llenado por COLUMNAS (el navegador baja la col 1 y sigue
  // en la 2). Filas = ceil(n/cols) para que cada columna quede a la misma altura.
  const gridColumnas = (n: number, cols: number): CSSProperties => ({
    display: "grid",
    gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))`,
    gridTemplateRows: `repeat(${Math.max(1, Math.ceil(n / cols))}, auto)`,
    gridAutoFlow: "column",
  });
    ...
    <div>
      ...
      {rol === "Profesor(a)" && !colegioId && (
        <div className="grid grid-cols-2 sm:flex sm:flex-row gap-2 sm:gap-3 mb-4">
          <select value={filtroNivelP} onChange={(e) => { setFiltroNivelP(e.target.value); setFiltroGradoP("todos"); setFiltroSalonP("todos"); }} className="flex h-10 w-full sm:w-52 rounded-md border border-input bg-card px-3 py-2 text-sm">
          ...
          <select value={filtroSalonP} onChange={(e) => setFiltroSalonP(e.target.value)} className="flex h-10 col-span-2 w-full sm:w-52 rounded-md border border-input bg-card px-3 py-2 text-sm">
              ...
              {asignaturasCol.length === 0 ? (
                ...
                <div>
                  ...
                  {asignaturasVisibles.length === 0 ? (
                    ...
                    <div className="border rounded-md p-2 mt-1 max-h-32 overflow-y-auto gap-x-4 gap-y-1.5" style={gridColumnas(asignaturasVisibles.length, esSm ? 2 : 1)}>
